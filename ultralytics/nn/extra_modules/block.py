...
class Fusion(nn.Module):
    def __init__(self, inc_list, fusion='bifpn') -> None:
        ...
        assert fusion in ['weight', 'adaptive', 'concat', 'bifpn', 'SDI', 'ajff']
        self.fusion = fusion
        ...
        if self.fusion == 'bifpn':
            ...
            self.SDI = SDI(inc_list)  # Assuming SDI is another module defined elsewhere
        elif self.fusion == 'ajff':
            self.fusion_conv1 = nn.ModuleList([Conv(inc, inc, 1) for inc in inc_list])
            self.fusion_conv2 = nn.ModuleList([Conv(inc, inc, 1) for inc in inc_list])
            self.fusion_adaptive = Conv(sum(inc_list), len(inc_list), 1)
            self.fusion_weight = nn.Parameter(torch.ones(len(inc_list), dtype=torch.float32), requires_grad=True)
            self.relu = nn.ReLU()
            self.epsilon = 1e-4
    ...
    def forward(self, x):
        ...
        if self.fusion == 'weight':
            ...
            for i in range(len(a)):
                a[i] = self.fusion_conv1[i](a[i])
            a = torch.sum(torch.stack(a, dim=0), dim=0)
            for i in range(len(b)):
                b[i] = self.fusion_conv2[i](b[i])
            fusion = torch.softmax(self.fusion_adaptive(torch.cat(b, dim=1)), dim=1)
            b_weight = torch.split(fusion, [1] * len(b), dim=1)
            b = torch.sum(torch.stack([b_weight[i] * b[i] for i in range(len(b))], dim=0), dim=0)
            fusion_weight = self.relu(self.fusion_weight.clone())
            fusion_weight = fusion_weight / (torch.sum(fusion_weight, dim=0))
            c = torch.sum(torch.stack([fusion_weight[i] * c[i] for i in range(len(c))], dim=0), dim=0)
            return torch.cat((a, b, c), dim=1)  # 按通道维数进行拼接
