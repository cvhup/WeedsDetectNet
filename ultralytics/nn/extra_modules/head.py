...
class Detect_yolov5(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):
        ...
        self.nc = nc  # number of classes
        self.nl = len(ch)  # number of detection layers
        # self.na = len(ch)  # number of anchors
        self.reg_max = 16  # DFL channels (ch[0] // 16 to scale 4/8/12/16/20 for n/s/m/l/x)
        self.no = nc + self.reg_max * 4  # number of outputs per anchor
        self.stride = torch.zeros(self.nl)  # strides computed during build
        c2, c3 = max((16, ch[0] // 4, self.reg_max * 4)), max(ch[0], min(self.nc, 100))  # channels
        ...
        # self.m = nn.ModuleList(nn.Sequential(Conv(x, c2, 3), Conv(c2, c3, 3), nn.Conv2d(c3, self.no, 1)) for x in ch)
        self.dfl = DFL(self.reg_max) if self.reg_max > 1 else nn.Identity()
    ...
    def bias_init(self):
        ...
        #     a[-1].bias.data[:] = 1.0  # box
        #     b[-1].bias.data[: m.nc] = math.log(5 / m.nc / (640 / s) ** 2)  # cls (.01 objects, 80 classes, 640 img)
        for b, s in zip(m.m, m.stride):  # from
            b.bias.data[: m.nc] = math.log(5 / m.nc / (640 / s) ** 2)  # cls (.01 objects, 80 classes, 640 img)
