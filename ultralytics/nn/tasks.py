...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (Detect, Detect_DyHead, Detect_yolov5, Detect_AFPN_P2345, Detect_AFPN_P2345_Custom, Detect_AFPN_P345, Detect_AFPN_P345_Custom, 
...
def attempt_load_weights(weights, device=None, inplace=True, fuse=False):
    ...
    for m in ensemble.modules():
        t = type(m)
        if t in (nn.Hardswish, nn.LeakyReLU, nn.ReLU, nn.ReLU6, nn.SiLU, Detect, Detect_DyHead, Detect_yolov5, Detect_AFPN_P2345, Detect_AFPN_P2345_Custom, Detect_AFPN_P345, 
...
def attempt_load_one_weight(weight, device=None, inplace=True, fuse=False):
    ...
    for m in model.modules():
        ...
        if t in (nn.Hardswish, nn.LeakyReLU, nn.ReLU, nn.ReLU6, nn.SiLU, Detect, Detect_DyHead, 
                 Detect_AFPN_P2345, Detect_AFPN_P2345_Custom, Detect_AFPN_P345, Detect_yolov5, 
...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, Focus,
                 ...
                 C3_RetBlock, C2f_RetBlock, G1012, GA, TransformerBlock):
            if args[0] == 'head_channel':
                args[0] = d[args[0]]
            c1, c2 = ch[f], args[0]
        ...
        # elif m is (AIFI, CBAMBlock):
        ...
        elif m in (Detect, Detect_DyHead, Detect_yolov5, Detect_AFPN_P2345, Detect_AFPN_P2345_Custom, Detect_AFPN_P345, Detect_AFPN_P345_Custom, 
            ...
            c1, c2 = [ch[x] for x in f], (sum([ch[x] for x in f]) if args[0] == 'concat' else (3 * ch[f[0]] if args[0] == 'ajff' else ch[f[0]]))
            args = [c1, args[0]]
        elif m is CBLinear:
            ...
            c2 = m.channel
                   ...
                   FocusedLinearAttention, LocalWindowAttention, ChannelAttention_HSFPN, ELA_HSFPN, CA_HSFPN, DySample, CARAFE, CBAMBlock}:
            c2 = ch[f]
            args = [c2, *args]
            # print(args)
...
#     return d
def yaml_model_load(path):
    """Load a YOLOv8 or WeedsDetectNet model from a YAML file."""
    import re
    path = Path(path)
    ...
    if re.match(r"(yolov\d+[nslmx])|(?:WeedsDetectNet[nslmx])", path.stem):
        unified_stem = re.sub(r"(\d+|WeedsDetectNet)([nslmx])(.+)?$", r"\1\3", path.stem)
        unified_path = path.with_name(unified_stem + path.suffix)
        LOGGER.info(f"Attempting to load model from generalized path: {unified_path}")
    else:
        unified_path = path

    yaml_file = check_yaml(unified_path, hard=False) or check_yaml(path)
    if yaml_file is None:
        raise FileNotFoundError(f"YAML file not found for path {path} or unified path {unified_path}")
...
def guess_model_scale(model_path):
    """Guess the model scale (n/s/l/m/x) from the model file name for YOLO and WeedsDetectNet models."""
    import re
    from pathlib import Path
    import contextlib

    with contextlib.suppress(AttributeError):
        # 匹配 YOLO 或 WeedsDetectNet，并返回 n/s/l/m/x 字符
        match = re.search(r"(?:yolov\d+|WeedsDetectNet)([nslmx])", Path(model_path).stem)
        if match:
            return match.group(1)  # 返回 n/s/l/m/x
...
def guess_model_task(model):
    ...
    if isinstance(model, nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, Detect, Detect_DyHead, Detect_yolov5, Detect_AFPN_P2345, Detect_AFPN_P2345_Custom, 
